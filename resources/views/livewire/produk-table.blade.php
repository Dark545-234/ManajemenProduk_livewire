<div>
    <div class="my-3 p-3 bg-body rounded shadow-sm">
        <div class="pb-3 pt-3 d-flex justify-content-between align-items-center">
            <input type="text" class="form-control w-25" wire:model.live="katakunci" placeholder="Cari produk...">
        ...
        <table class="table table-striped table-sortable">
            <thead>
                <tr>
                    ...
                    <th class="col-md-2 sort @if ($sortcolum == 'nama'){{ $sortarah }} @endif" wire:click="sort('nama')">Nama Produk</th>
                    <th class="col-md-1 sort @if ($sortcolum == 'harga'){{ $sortarah }} @endif" wire:click="sort('harga')">Harga</th>
                    <th class="col-md-1 sort @if ($sortcolum == 'stok'){{ $sortarah }} @endif" wire:click="sort('stok')">Stok</th>
            ...
            <tbody>
                @forelse ($dataProduks as $key => $value)
                    <tr>
                        ...
                        <td>{{ $value->nama }}</td>
                        <td>Rp{{ number_format($value->harga, 0, ',', '.') }}</td> {{-- Format harga agar lebih mudah dibaca --}}
                        <td>{{ $value->stok }}</td>
                        <td>{{ $value->supplier->nama ?? '-' }}</td> {{-- Akses nama supplier melalui relasi --}}
                        <td>{{ $value->kategori->nama ?? '-' }}</td> {{-- Akses nama kategori melalui relasi --}}
                        <td>
